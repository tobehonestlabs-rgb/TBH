...
function countryFlag(code: string | null | undefined): string {
  ...
  return String.fromCodePoint(offset + code.charCodeAt(0) - 65, offset + code.charCodeAt(1) - 65)
}
...
export default function MessagesPage({ onUnreadChange, isActive }: Props) {
  ...
  const openSheet = async (msg: Message) => {
    if (!msg.isOpened) {
      // Properly await to ensure DB update fires and completes
      await supabaseClient.from('messages').update({ isOpened: true }).eq('message_id', msg.message_id)
  ...
  const closeSheet = () => {
    ...
    setShowFullscreen(false); setShowConv(false)
    setTimeout(() => {
      setSelectedMsg(null); setSheetClosing(false); setShowReply(false)
  ...
  const startConversation = async () => {
    ...
    try {
      const res = await fetch('/api/conversations', {
        ...
        body: JSON.stringify({ original_message_id: selectedMsg.message_id }),
      })
  ...
  const sendConvMsg = async () => {
    ...
    try {
      ...
      if (message) setConvMsgs(prev => [...prev, message])
  ...
  return (
    <>
      ...
      {/* ── Full-screen message sheet ── */}
      {selectedMsg && portalTarget && createPortal(
        <div className="fixed inset-0 z-50 flex flex-col">
          <div className={`flex-1 ${sheetClosing ? 'sheet-exit' : 'sheet-enter'} bg-white flex flex-col overflow-hidden`} style={{ borderRadius: '28px 28px 0 0' }}>
            {/* Top bar */}
            ...
            {/* Scrollable content — slides between main and insights */}
            <div className="flex-1 overflow-y-auto">
              {/* ── Main message view ── */}
              {!showInsights && !showConv && (
                <div className="px-5 pt-4 pb-4 slide-from-left">
                  {!showReply ? (
                    <>
                      {isImageMessage && imageUrl && (
                        <>
                          <button onClick={() => setShowFullscreen(true)} className="w-full flex items-center gap-3 p-3 rounded-[18px] active:scale-[0.98] transition-transform mb-4" style={{ background: '#F7F7F9' }}>
                            ...
                            <button onClick={e => { e.stopPropagation(); setImageBlurred(b => !b) }} className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 active:scale-90 transition-transform" style={{ background: imageBlurred ? '#0D0D0D' : '#E8E8E8' }}>
                              {imageBlurred ? (
                                <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" stroke="white" strokeWidth="2"/><circle cx="12" cy="12" r="3" stroke="white" strokeWidth="2"/></svg>
                              ) : (
                                <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24" stroke="#0D0D0D" strokeWidth="2" strokeLinecap="round"/><line x1="1" y1="1" x2="23" y2="23" stroke="#0D0D0D" strokeWidth="2" strokeLinecap="round"/></svg>
                              )}
                            </button>
                          </button>
                      ...
                      {textContent && (
                        <div className="w-full rounded-[24px] p-6 mb-4 flex items-center justify-center" style={{ background: '#F7F7F9', minHeight: 120 }}>
                          <p className="text-[#0D0D0D] font-semibold text-center leading-snug" style={{ fontSize: textContent.length > 100 ? '18px' : textContent.length > 50 ? '22px' : '28px' }}>
                            {textContent}
                          </p>
                        </div>
                      )}

                      <p className="text-[11px] text-[#ADADAD] text-center mb-5">{timeAgo(selectedMsg.created_at)}</p>

                      {/* Who sent this? */}
                      <button onClick={loadInsights} className="w-full flex items-center justify-between px-4 py-3 rounded-[16px] mb-3 active:scale-[0.98] transition-transform" style={{ background: '#F7F7F9' }}>
                        <div className="flex items-center gap-2.5">
                          <span className="text-[18px]">🕵️</span>
                          <div className="text-left">
                            <p className="text-[14px] font-bold text-[#0D0D0D]">Who's sent that??</p>
                            <p className="text-[11px] text-[#ADADAD]">See sender insights</p>
                          </div>
                        </div>
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6" stroke="#CCC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </button>

                      {/* Start a conversation */}
                      <button onClick={startConversation} className="w-full flex items-center justify-between px-4 py-3 rounded-[16px] mb-5 active:scale-[0.98] transition-transform" style={{ background: '#0D0D0D' }}>
                        <div className="flex items-center gap-2.5">
                          <span className="text-[18px]">💬</span>
                          <div className="text-left">
                            <p className="text-[14px] font-bold text-white">Start a conversation</p>
                            <p className="text-[11px] text-white/50">Reply privately</p>
                          </div>
                        </div>
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => setShowReply(false)} className="flex items-center gap-1.5 text-[#888] text-[13px] mb-4 active:opacity-60">
                        <svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7" stroke="#888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        Back
                      </button>
                      {textContent && <div className="w-full rounded-[16px] px-4 py-3 mb-4" style={{ background: '#F7F7F9' }}><p className="text-[#555] text-[14px] leading-snug line-clamp-3">{textContent}</p></div>}
                      <textarea value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Type your reply..." autoFocus rows={4} className="w-full rounded-[16px] bg-[#F7F7F9] px-4 py-3 text-[16px] text-[#0D0D0D] outline-none resize-none mb-4" style={{ fontFamily: 'inherit' }} />
                      <button onClick={handleSendReply} disabled={!replyText.trim()} className="w-full py-[15px] rounded-full bg-[#0D0D0D] text-white font-bold text-[15px] active:scale-95 transition-transform disabled:opacity-40">Send Reply</button>
                    </>
                  )}
                </div>
              )}
              ...
              {/* ── Insights panel ── */}
              {showInsights && !showConv && (
                <div className="px-5 pt-4 pb-6 slide-from-right">
                  <button onClick={() => setShowInsights(false)} className="flex items-center gap-1.5 text-[#888] text-[13px] mb-5 active:opacity-60">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7" stroke="#888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    Back to message
                  </button>

                  <p className="text-[22px] font-extrabold text-[#0D0D0D] mb-1">Sender Insights</p>
                  <p className="text-[13px] text-[#ADADAD] mb-5">Approximate information about who sent this</p>

                  {/* Info cards */}
                  <div className="flex flex-col gap-3 mb-4">
                    {/* Country + City */}
                    <div className="rounded-[20px] p-4" style={{ background: '#F7F7F9' }}>
                      <p className="text-[11px] font-semibold text-[#ADADAD] uppercase tracking-wider mb-2">Location</p>
                      <div className="flex items-center gap-2">
                        <span className="text-[28px]">{countryFlag(selectedMsg.country)}</span>
                        <div>
                          <p className="text-[16px] font-bold text-[#0D0D0D]">
                            {[selectedMsg.city, selectedMsg.country].filter(Boolean).join(', ') || 'Unknown'}
                          </p>
                          {selectedMsg.region && <p className="text-[12px] text-[#ADADAD]">{selectedMsg.region}</p>}
                        </div>
                      </div>
                    </div>
                    ...
                    {/* IP */}
                    <div className="rounded-[20px] p-4" style={{ background: '#F7F7F9' }}>
                      <p className="text-[11px] font-semibold text-[#ADADAD] uppercase tracking-wider mb-1">IP Address</p>
                      <p className="text-[15px] font-mono font-bold text-[#0D0D0D]">
                    ...
                    {selectedMsg.browser_name && (
                      <div className="rounded-[20px] p-4" style={{ background: '#F7F7F9' }}>
                        <p className="text-[11px] font-semibold text-[#ADADAD] uppercase tracking-wider mb-1">Browser</p>
                        <p className="text-[15px] font-bold text-[#0D0D0D]">{selectedMsg.browser_name}</p>
                      </div>
                    )}

                    {/* Message count */}
                    <div className="rounded-[20px] p-4" style={{ background: '#F7F7F9' }}>
                      <p className="text-[11px] font-semibold text-[#ADADAD] uppercase tracking-wider mb-1">Messages from this sender</p>
                      <p className="text-[24px] font-extrabold text-[#0D0D0D]">
                  ...
                  {/* Map */}
                  {mapUrl ? (
                    <div className="w-full rounded-[20px] overflow-hidden mb-5" style={{ height: 180 }}>
                      <iframe src={mapUrl} className="w-full h-full border-0" title="Approximate location" />
                    </div>
                  ) : (
                    <div className="w-full rounded-[20px] mb-5 flex items-center justify-center" style={{ height: 100, background: '#F7F7F9' }}>
                      <p className="text-[13px] text-[#ADADAD]">Location map not available</p>
                    </div>
                  )}

                  {/* Start conversation */}
                  <button onClick={startConversation} className="w-full py-[16px] rounded-full font-bold text-[16px] text-white active:scale-95 transition-transform" style={{ background: '#0D0D0D' }}>
                    💬 Start a conversation
                  </button>
                </div>
              )}
              ...
              {showConv && (
                <div className="flex flex-col h-full slide-from-right">
                  <div className="px-5 pt-2 pb-2 border-b border-[#F0F0F0]">
                    <button onClick={() => setShowConv(false)} className="flex items-center gap-1.5 text-[#888] text-[13px] py-1 active:opacity-60">
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7" stroke="#888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      Back
                    </button>
                    <p className="text-[17px] font-bold text-[#0D0D0D] mt-1">Anonymous sender</p>
                    <p className="text-[12px] text-[#ADADAD]">They'll see your messages if they sign up</p>
                  </div>
                  ...
                  {/* Input */}
                  <div className="px-4 pb-6 pt-2 border-t border-[#F0F0F0] flex gap-2">
                    <input
                      value={convInput} onChange={e => setConvInput(e.target.value)}
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendConvMsg() } }}
                    ...
                    <button onClick={sendConvMsg} disabled={!convInput.trim() || convSending} className="w-10 h-10 rounded-full bg-[#0D0D0D] flex items-center justify-center active:scale-90 transition-transform disabled:opacity-40 flex-shrink-0">
                      <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M22 2 11 13M22 2l-7 20-4-9-9-4 20-7z" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </button>
                  </div>
                </div>
              )}
            ...
            {/* Bottom action buttons — only on main non-reply view */}
